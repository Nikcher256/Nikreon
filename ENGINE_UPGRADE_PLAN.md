...
- Added an inspector color-swatch button with a nearby compact Unreal-inspired popup: smooth saturation/value palette, hue strip, old/new previews, gradient RGB fine-tuning tracks, editable `0-255` RGB fields, and a synchronized `#RRGGBB` hex field. The editor background leaves the viewport target visible so the selected clear color can be checked interactively beneath editor overlays.
